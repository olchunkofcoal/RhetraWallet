...
void WalletMain::setOnline(bool pbFlag, const QString& psReason)
{
    ...
    this->setWindowTitle(QString("Rhetra Wallet [%1%2]").arg(strAccountID).arg(pbFlag ? "" : ", offline"));
...
void WalletMain::socketConnect()
{
    ...
    // Connect to random Rhetra server
    vector<QString> servers = {"wss://david.rhetra.io:443/",};
...
void WalletMain::submitResponse(const QJsonObject& poResp)
{
    ...
    if (result["status"] == "error")
        Show("Transaction error", QString("Failure while committing transaction to the Rhetra network: ") + poResp["error_message"].toString(), E_WARN);
    else if (result["engine_result"].toString() != "tesSUCCESS")
        Show("Transaction error", QString("Error while processing transaction by the Rhetra network: ") + result["engine_result_message"].toString(), E_WARN);
    else
    ...
    if (eNone == eRes)
    {
        ...
        if (pbJustAsk)
        {
            QString strConf = "I'm about to send " + ui->amountToSend->text()
                    + " RHET to " + ui->receiverAddressEdit->text() + ". Do you agree?";
...
void WalletMain::on_actionProxy_triggered()
{
    ...
    if (proxysettings.exec() == QDialog::Accepted)
    {
        ...
        RhetraSettings.Write(CIniworker::W_PROXY);
        doReconnect();
    };
}
...
{
    RhetraSettings.Read(CIniworker::R_PROXY);
